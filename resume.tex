...
\CN{
 ...
 \CN{
    ...
    \begin{itemize}[leftmargin=*, parsep=0.5ex, itemsep=0.5ex]
      ...
      \item \textbf{数据库与缓存}：\textbf{熟练 PostgreSQL 设计与优化}，具备 \textbf{Redis 在高并发场景下的经验}（缓存击穿/雪崩/穿透）。
      \item \textbf{消息队列与事件驱动}：熟练使用 \textbf{NATS}，具备\textbf{事件驱动架构}设计经验，掌握\textbf{异步消息处理}。
      \item \textbf{工程与部署}：熟悉 \textbf{Linux} 与 \textbf{Git}，具备\textbf{从开发到部署的全流程交付能力}。
  ...
  \datedsubsection{\textbf{Doc2X — C/B端文档解析平台}，后端开发}{2025.06 - 至今}
    \begin{itemize}[parsep=0.5ex]
      ...
      \item \textbf{贡献亮点}：
        ...
        \item {\textbf{多渠道通知系统}}：设计并实现\textbf{告警通知系统}，采用\textbf{策略模式 + 工厂模式}接入多个通知渠道，支持余额告警、资源告警、系统公告等场景。
        \item {\textbf{消费者服务迭代}}：围绕 \textbf{NATS} 构建解析、翻译、订单、奖励等事件处理链路，统一实现 \textbf{用户额度扣减}、\textbf{限流幂等校验} 与 \textbf{结果缓存} 更新，保证多业务场景的配额与缓存一致性。
        \item {\textbf{运营后台迭代}}：围绕 Admin 端整合 \textbf{公告投放}、\textbf{发票审核}、\textbf{退款审批} 与 \textbf{用户订单/消费查询} 等核心流程，帮助运营与财务在一个后台完成日常审核、对账与运营动作。
        \end{itemize}
    \end{itemize}
 ...
 \begin{itemize}[parsep=0.5ex]
   ...
   \item \textbf{贡献亮点}：
     \begin{itemize}[leftmargin=*, parsep=0pt, itemsep=0.5ex]
       ...
       \item \textbf{性能优化}：实现\textbf{多租户备份服务}，结合 \textbf{MapReduce 思想}与\textbf{协程优化}，\textbf{响应时间从 30 秒缩短至 2 秒}，并沉淀为可复用工具。
       \item \textbf{容器服务（网关）}：将 Java 命令式容器服务改造为 \textbf{Kubernetes 声明式架构}，统一资源模型，\textbf{核心接口响应时间从 20 秒降至 300 毫秒}，显著改善用户体验。
